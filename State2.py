...
class State2(AiManager):
    ...
    def __init__(self, publisher:Publisher):
        ...
        self.memory = []
        #depends on model
        self.directory = Path("./State2_output/")
    ...
    def generateState(self, msg:StatePb):
        ...
        if 'Tracks' not in json_dict:
            ...
            execution_order = self.calculateExecutionOrder(json_dict['Tracks'], json_dict['assets'],origin)
            output_message: OutputPb = OutputPb()
    ...
    def whoShootsFirst(self, assets, target_id)-> tuple:
        ...
        for asset in assets:
            if asset['health'] == -1:
                pass
            #elif asset['AssetName'] == 'HVU_Galleon_0':

            else:
                for weapon in asset['weapons']:
                    print(weapon)
                    #BUG: quantity sometimes not given by the pla1nner
    ...
    @staticmethod
    def calculateExecutionOrder(missle_list, assets, origin) -> list: #of tuples
        # t = (s/m) * m = s
        intercept_times=[]
        ...
        for missle in missle_list:
            ...
            for asset in assets:
                if asset['health'] == -1:
                    ...
                    rel_pos = [x-asset['PositionX'], y-asset['PositionY']]
                    rel_vel = [vx, vy]

                    rel_pos_norm = [rel_pos[0]/math.sqrt(rel_pos[0]**2 + rel_pos[1]**2), rel_pos[1] / math.sqrt(rel_pos[0]**2 + rel_pos[1]**2)]
                    rel_vel_norm = [rel_vel[0] / math.sqrt(rel_vel[0] ** 2 + rel_vel[1] ** 2), rel_vel[1] / math.sqrt(rel_vel[0] ** 2 + rel_vel[1] ** 2)]
                    dot_prod = rel_pos_norm[0]*rel_vel_norm[0]+rel_pos_norm[1]*rel_vel_norm[1]

                    if dot_prod < 0:
                        print(asset['AssetName']+" Targeted By "+str(missle['TrackId']))

                    #velocity eqn
                    dv = np.sqrt(vx**2 + vy**2)
                    #print("Velocity " +str(dv))
                    #time to intercept
                    t = d/dv
